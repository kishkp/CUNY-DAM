...
# Below code takes too much time. Hence it is commented. But this data has been used in the Analysis.
...
# BSE data from csv and requisite transformations

BSE_raw <-read.csv("https://raw.githubusercontent.com/kishkp/CUNY-DAM/master/FinalProject/MasterData/BSEDMAData.csv", stringsAsFactors = FALSE, check.names = FALSE, na.strings = c("", "NA"), header = TRUE)

# convert date to match the other dates
BSE_raw$TradeDate <- as.Date(BSE_raw$TradeDate,  "%d-%B-%y")
...
LeadManager_IPO_List$IssuerCompany <- str_replace(LeadManager_IPO_List$IssuerCompany, " IPO", "")


# Identify all LMs who have less than 20 and group them under others

IssuesByLM<-LeadManager_IPO_List %>% group_by(LMName) %>% summarise(Issues=n())
IssuesByLM <- as.data.frame(as.matrix(IssuesByLM),stringsAsFactors=F)
IssuesByLM <- cbind(IssuesByLM, NewLMName=ifelse(IssuesByLM[,2]<20, "Others", IssuesByLM$LMName))
LeadManager_IPO_List <- left_join(LeadManager_IPO_List, IssuesByLM[,c(1,3)], by="LMName" )
...
dbWriteTable(mydb,"BSEData",BSE_raw,overwrite=T)
...
# #Calculate Trend based on selected DMA and store as another column
# #Note: Trend for TODAY is Previous day Close > previous day 7DMA
# BSE_raw <- cbind(BSE_raw, Trend = ifelse(lag(BSE_raw$Close) - lag(BSE_raw$`DMA7`)>0, "Positive", "Negative"))
# 
# 
# # Calculate a Successful or Unsuccessful IPO based on selected cutoff
# IPO_OverSub_ListingGains <- cbind(IPO_OverSub_ListingGains, IPOresult = ifelse(IPO_OverSub_ListingGains$ListingGains>=7, "Successful", "Unsuccessful"))





# Registrar - UNUSED data
# 
# mydb = dbConnect(MySQL(), user=proj_user, password=proj_pwd, dbname=proj_db, host=proj_host)
